 ...
 * @param iopaPrefix (string)  the IOPA  prefix to search for (e.g., "iopa.")
 * @param iopaObject (object)  the javascript object on which to add the prototypes (e.g., context.Request)
 * @returns (void)
 * @private
 */
...
function expandContext(context, addReqRes) {
    ...
    {
        console.log("IOPA app running " + this.properties["server.AppId"]);
        
        // add default aliases to IopaContext if needed;  not currently in default IOPA spec
        // iopaExpandHelpers.refreshPrototypeIopaContext(context);
        
        Object.defineProperty(context.constructor.prototype, "server.AppId", {value : this.properties["server.AppId"],
        ...
        if (isIopaNative)
            initIopaNativeContextPrototype(context.constructor.prototype);
    }
...
/**
 * Representss an IOPA server Object
 *
 * @class IopaServer
 * @constructor
 */
...
function initIopaNativeContextPrototype(contextPrototype){
    ...
    contextPrototype.response.protoype["iopa.WriteHead"] = function iopa_WriteHead(statusCode, headers)
    {
    ...
    contextPrototype.response.protoype["iopa.SetHeader"] = function iopa_SetHeader(key, val)
    {
        private_setIgnoreCase(this["iopa.Headers"], key, val);
    }
    
    contextPrototype.response.protoype["iopa.GetHeader"] = function iopa_GetHeader(key)
    {
        return private_getIgnoreCase(this["iopa.Headers"], key);
    }
    
    contextPrototype.response.protoype["iopa.RemoveHeader"] = function iopa_RemoveHeader(key, value)
    {
...
{
    var proto = context.constructor.prototype;
