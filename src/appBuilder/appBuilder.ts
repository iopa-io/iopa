...
export default class AppBuilder {
    ...
    constructor(
        ...
        const defaults: any = new AppPropertiesWithCapabilities<
            AppCapabilities
        >({
            'server.AppId': guid(),
    ...
    public use(method: 'invoke' | 'dispatch', mw?: any): this {
        ...
        if (params === 'app' || mw.length === 1) {
            ...
            if (typeof mwInstance.dispatch === 'function') {
                this.middleware.dispatch.push(
                    mwInstance.dispatch.bind(mwInstance)
                )
            }
        } else {
    ...
    public build() {
        ...
        if (this.middleware.dispatch.length > 0) {
            pipeline.dispatch = this.compose_(
                this.middleware.dispatch.reverse()
            )
        } else {
        ...
        return (context: IopaContext) => {
            ...
            context.capability =
                ...
                ((keyOrRef: keyof AppCapabilitiesBase | IopaRef<any>) => {
                    ...
                    return this.properties.get('server.Capabilities')[
                        keyOrRef.id
                    ]
                })
            ...
            context.set =
                context.set ||
                ((
                    data: string | IopaMapInit<Record<any, any>>,
                    value?: any
                ) => {
