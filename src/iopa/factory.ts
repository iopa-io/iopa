...
> {
    ...
    public createContext(
        ...
        if (optionsValid.withResponse) {
            ...
            request.set('iopa.Headers', new IopaHeaders())
            ...
            response.set('iopa.Headers', new IopaHeaders())
            ...
            response.end = async (body, sendoptions) => {
                response.send(body, sendoptions)
    ...
    private _dispose(
        context: T & IopaRequestBase & { response: IopaResponse }
    ) {
        if (
            context == null ||
            context.get('server.CancelTokenSource') == null
        ) {
            return
        }
